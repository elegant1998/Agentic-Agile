# 第二十二章｜流程与工具重构：从旧流程加 AI 到 AI 原生价值流
...
这条价值流不是新的阶段瀑布。S、C、O 持续控制方向、边界和协作，P⇄E 快速收敛，V 保持独立，Telemetry 跨任务反馈。五道机械门只裁决状态转换，不增加五层审批。

### 22.3.1 从文档流转变成控制状态转换

| 控制状态 | 进入条件 | 机器可以做什么 | 必须停止的情况 |
|---|---|---|---|
| 意图探索 | 目标尚未签署 | 检索、分析、提问、生成选项 | 不得创建业务实现或扩大权限 |
| 可执行准备 | 契约、约束和 AC 已成立 | 裁剪 Context、规划 Work Graph、准备测试 | 签署缺失、UNKNOWN 影响目标或边界 |
| 受控执行 | 前置门和编码门满足 | 在授权文件、工具和预算内实现与验证 | 越界、MUST 失败、不可逆操作、预算耗尽 |
| 证据收敛 | P⇄E 达到退出条件 | 汇聚 Evidence、检查新鲜度和一致性 | 证据矛盾、关键 AC 未证明、回退不可用 |
| 责任裁决 | V 具备独立审查条件 | 提出批准、条件批准、补证或拒绝建议 | 机器不得替有权者接受残余风险 |
| 反馈捕获 | 裁决与任务事实已记录 | 生成单任务遥测并进入跨任务聚合 | 不得用一次任务宣称组织趋势 |

状态转换比岗位交接更适合高速 Agent：它说明当前事实允许什么行动，而不是规定某份文件必须在某个部门停留多久。岗位仍承担责任，但流程由可验证状态驱动。

### 22.3.2 HITL 是异常路由，不是默认收费站

人类介入应绑定事件、责任资格和决策时限。例如规格冲突路由给 IO，权限例外路由给安全或数据责任人，跨模块责任冲突路由给相应 Decision Owner，证据不足路由给独立验证者。通知必须同时携带问题、证据、影响、选项和推荐理由，不能只发一句“请审批”。

组织还要观察 HITL 的队列时间和重复类型。大量相同低风险问题反复找人，说明规则或工具能力没有产品化；重大风险从不触发 HITL，则更可能是漏报而不是成熟。

## 22.4 工具从 Copilot 孤岛走向最小工程系统
