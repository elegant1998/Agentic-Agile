...
三大自治机制规定的是运行逻辑：意图注入使执行从已签署的目标和边界开始；高频对抗与自净化使偏差在有限范围内暴露、修复或停止；人类异常裁决处理价值取舍、例外与不可逆行动。

Harness 规定这些逻辑依赖的运行能力。意图注入需要可信上下文、状态和约束；高频对抗与自净化需要执行编排、评估、预算和停止条件；人类异常裁决需要工具权限、证据、状态与恢复。它们不是一一对应的三套模块，而是共同依赖六类能力域。
