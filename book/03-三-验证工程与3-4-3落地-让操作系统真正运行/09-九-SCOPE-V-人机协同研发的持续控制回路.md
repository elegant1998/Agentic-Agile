...
前面的三大理论解释了 3-4-3 为什么需要外部化认知、建立反馈并治理复杂协作；三大自治机制则说明系统怎样获得意图、纠正偏差并把机器不该决定的事情交还人类。但这些原则还需要一套能够真正运行的控制架构。SCOPE-V 正是连接方法论与工程执行的中枢。

本章先给出 SCOPE-V 的整体结构，再讨论它与传统 SDLC 的关系、六个控制面、快内环与慢外环、机械门禁，以及三大自治机制怎样通过这条控制回路落地。后续各章再分别展开 Specify、Constrain、Orchestrate、Prove、Evolve，以及 Verify 与 Telemetry。
...
## 9.13 三大自治机制怎样通过 SCOPE-V 落地

三大自治机制与 SCOPE-V 处在不同的抽象层，不能简单一一对应，也不应被理解成两套并列框架。

- **三大自治机制**描述系统怎样运行：如何获得意图、如何纠偏、何时把决定交还人类；
- **SCOPE-V**描述运行时的控制面：目标、边界、编排、证明、演进与裁决分别由什么环节承担；
- **六大 Harness 支柱**提供工程条件：上下文、工具、图、状态、评估、约束与恢复怎样真正执行。

可以把它们理解为“运行原则—控制回路—基础设施”的关系：
...
SCOPE-V：自治在哪些控制面中发生
        ↓ 依托
六大 Harness 支柱：控制怎样被机器执行和观测
```

因此，SCOPE-V 不是三大机制之后才启动的一段流程，也不是额外的第四个自治机制。每一种机制都横跨 SCOPE-V 的多个控制面。

### 9.13.1 机制一：从意图注入到可执行起点

意图注入主要发生在 S、C、O 三个控制面，但它不是简单地“依次填三份表”。

**Specify 控制注入什么目标。** OA 从 Intent Graph 中裁剪相关背景，通过批判性思维、Grill-Me 和 Example Mapping，把 IO 的模糊意图转成 Goal、Non-goals、Rules 和 AC。IO 对完整 Intent Contract 显式签署，建立执行授权。

**Constrain 控制注入哪些边界。** 已签署契约只说明本次要做什么，Constraint Matrix、AI Coding Guide 和 Tools Manifest 还要说明不能做什么、可以使用什么工具、遇到风险怎样停止。目标与边界共同组成 Agent 的任务上下文。

**Orchestrate 控制把意图注入给谁、在什么时候注入。** OA 不会把整个项目知识库复制给所有 Agent，而是根据 Work Graph 为不同节点裁剪不同 Context。测试 Agent 获得 AC、接口和约束；实现 Agent 获得签署规则、可信 Red 和允许修改范围；验证节点获得原始证据，但不必继承实现 Agent 的全部推理。
...
前置门是这套机制的机械保证。契约不存在、IO 未真实签署、约束缺失或 AC 不可执行时，意图注入就没有完成，Orchestrate 不得进入业务编码。

### 9.13.2 机制二：P⇄E 快内环中的高频对抗与自净化

高频对抗与自净化最集中地体现在 P⇄E 快内环中，但 S、C、O 始终在外围提供目标、边界和执行结构。

**Prove 负责制造可信挑战。** 它从签署 AC 生成测试，通过 TDD Red 证明测试能够发现缺失行为，再用边界、异常、安全、性能和约束检查挑战候选实现。P 不是为了尽快得到绿灯，而是为了尽早暴露偏差。

这里的“挑战”不仅包括功能测试。SAST、SCA、Secret Scan、IaC Scan、DAST、Fuzzing，以及经过授权的红队攻击，都可以成为 P 的误差信号；E 根据发现完成代码、配置、权限、检测与恢复能力的最小修复，再由 P 使用原规则、原攻击路径或等价强度的测试重新取证。区别在于反馈速度与独立性：SAST 等确定性扫描可以高频进入每次提交，DAST 需要可运行环境，红队与渗透测试还需要独立授权、隔离范围和证据保全，不能全部压成同一种自动循环。

**Evolve 负责分类与受控修复。** 同样一个测试失败，可能来自实现错误、规格歧义、约束冲突、依赖异常或证据缺口。E 先判断失败应该回到哪里，再决定是否允许 AS 在当前授权范围内最小修复。

**修复后必须回到 P 重新取证。** E 不能自己声称已经修好，也不能换一个更宽松的检查。原测试、原约束和相关回归必须再次运行，形成从 Red 到 Green 的证据链。
...
如果失败需要改变契约，就回到 S；需要放宽或新增红线，就回到 C；需要调整依赖、超时或责任节点，就回到 O；涉及不可逆操作或超出自治预算，就转入机制三的人类裁决。

因此，高频自净化不是只属于 E。P 提供真实误差信号，E 负责纠偏，S/C/O 决定纠偏还能否在原授权内继续。三者共同保证内环快速但不失控。

### 9.13.3 机制三：人类裁决是横跨全程的异常出口

人类异常裁决不只发生在最后的 Verify。它是 SCOPE-V 每个控制面都可以触发的异常出口：

| 控制面 | 典型 HITL 触发 | 人类需要决定什么 |
|---|---|---|
| S | 目标冲突、价值取舍、规则歧义 | 做什么、不做什么、怎样算成功 |
| C | 法律安全冲突、权限扩大、约束例外 | 哪条边界优先、是否授权例外 |
| O | 多模块责任冲突、不可安全回滚 | 怎样重排、暂停或拆分任务 |
| P | 证据互相矛盾、自动判断不确定 | 还需要什么独立验证 |
| E | 超过重试预算、修复需要改契约 | 停止、重签、降级还是放弃 |
| V | 残余风险、灰度范围、生产发布 | 批准、条件批准、退回或拒绝 |
| Telemetry | 长期退化、成本异常、HITL 激增 | 收缩还是扩大自治范围 |

Verify 是人类裁决最集中的位置，因为它负责独立审查与发布建议；但如果把所有问题都拖到 V 才找人，Agent 可能已经在错误方向上消耗大量算力并扩大修改范围。

HITL 的设计原则是“事件触发”，不是“阶段打卡”。系统一旦检测到需要人承担的价值、法律、伦理、不可逆或例外责任，就应冻结相关节点、保存 Evidence、说明可选方案，并把决定交给正确责任人。

### 9.13.4 Verify 与 Telemetry 如何把三个机制闭成完整回路

P⇄E 收敛后，Verify 不属于高频自净化的一部分，而是站在内环之外检查三个问题：

1. 注入的意图是否仍与签署契约一致；
2. 自净化过程是否始终遵守约束，没有通过弱化标准获得全绿；
3. 所有需要人类裁决的事件是否由正确责任主体处理并留下记录。

Verify 通过后，Telemetry 将三大机制的运行质量变成数据：

- 目标准确率反映意图注入是否有效；
- 首次成功率、修复轮次和自愈率反映高频对抗是否收敛；
- HITL 升级率与类型反映人类裁决是否发生在正确位置；
- Token 与执行效率反映自治成本；
- 知识沉淀率反映失败是否真正反哺下一轮。

这些数据通过慢外环返回下一轮 S、C、O。若阈值歧义反复触发 HITL，S 应在 Grill-Me 中提前询问；若越权失败增加，C 应收紧 Tools Manifest；若 Token 和交接摩擦上升，O 应调整上下文裁剪与 Work Graph。
...
| 自治机制 | 主要 SCOPE-V 控制面 | 核心工件与证据 | 关键机械门 |
|---|---|---|---|
| 意图注入 | S、C、O | Intent Graph、Intent Contract、Constraint Matrix、任务 Context、Work Graph | 前置门 |
| 高频对抗与自净化 | O、P、E | TDD Red/Green、AC 结果、约束失败、修复记录、Loop Memory | 编码门、验证门 |
| 人类异常裁决 | 贯穿 S/C/O/P/E，集中于 V | 决策记录、例外批准、Evidence Bundle、发布裁决 | 前置门、验证门、收尾门、Bug 回溯门 |
| 长期反馈（贯穿三机制） | Telemetry → S/C/O | 单契约遥测、Dashboard、趋势与知识回写 | 收尾门、Bug 回溯门 |

这张表也解释了为什么三大机制不能离开 SCOPE-V 单独运行。没有 S/C/O，意图注入只是一次 Prompt；没有 P/E，高频对抗只是反复生成；没有独立 V 和 HITL，人类裁决只是最后盖章；没有 Telemetry，组织无法知道三种机制是在改善还是退化。

## 9.14 不要把三大机制机械映射成三个阶段

理解三大机制与 SCOPE-V 的关系后，还要避免三个看似直观但错误的映射。

### 9.14.1 误解一：意图注入只等于 Specify

Specify 负责形成签署目标，但如果没有 Constrain 提供边界、没有 Orchestrate 按节点裁剪并分发 Context，Agent 仍然只是拿到一份较长的需求文档。完整意图注入至少覆盖 S、C、O，并由前置门确认可以进入执行。

### 9.14.2 误解二：高频对抗只等于 Prove

Prove 负责产生反馈，Evolve 负责诊断与修复，随后必须回到 Prove 重验。与此同时，S/C/O 仍然规定修复目标、允许范围和预算。只有 P 没有 E，系统只会不断报错；只有 E 没有可信 P，系统可能在错误信号上自我优化。

### 9.14.3 误解三：人类裁决只等于 Verify

Verify 是发布裁决的集中位置，但人类责任贯穿全部控制面。目标冲突应在 S 解决，约束例外应在 C 解决，不可安全回滚的编排应在 O 解决，超过修复预算应在 E 停止。把全部问题拖到 V，只会让早期错误以更高成本到达终点。

正确理解不是寻找一张简单的一对一对应表，而是看每种机制怎样借助多个控制面完成闭环。三大机制定义运行原则，SCOPE-V 将原则分解为可执行控制，Harness 再把控制落实为工具、权限、门禁与证据。

## 9.15 三大机制如何通过 SCOPE-V 处理同一次失败

费用报销任务中，P 的边界测试发现 5000.00 元错误进入总监审批。

1. **意图注入机制提供判定基准**：S 中的签署契约明确“严格大于 5000 元”，C 禁止通过修改契约或删除测试获得通过；
2. **高频对抗机制产生反馈**：P 保存失败输入、期望、实际值和测试日志；
3. **E 执行自净化**：判断为实现比较符错误，在原文件范围和轮次预算内完成最小修复；
4. **P 重新取证**：重跑 4999.99、5000.00、5000.01 三个边界测试及相关回归；
5. **若发现制度冲突则触发机制三**：系统停止，OA 整理旧制度、当前契约和失败证据，由 IO 与财务责任人决定是否重签；
6. **V 独立裁决**：确认最终实现没有改变签署目标，自净化未突破约束，证据属于当前提交；
7. **Telemetry 记录机制表现**：首次成功率、修复轮次、自愈次数、HITL、Token 和知识回写进入单契约 Dashboard。

同一个失败在这里不是“测试失败—改代码—测试通过”三步，而是三大自治机制通过 SCOPE-V 协同运行的一次完整治理事件。
