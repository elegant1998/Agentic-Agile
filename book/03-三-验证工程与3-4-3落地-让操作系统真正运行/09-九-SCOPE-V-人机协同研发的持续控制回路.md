...
## 9.13 三大自治机制与 SCOPE-V 的接口

三大自治机制与 SCOPE-V 不是两套并列流程，也不存在一一对应关系：

| 层次 | 它回答的问题 |
|---|---|
| 3-4-3 | 谁负责、哪些工件承载、系统具备什么自治原则？ |
| 三大自治机制 | Agent 如何获得授权、接受纠偏、在越界时交还人类？ |
| SCOPE-V | 这些原则在一次任务的哪些控制面中运行？ |
| Harness | 上述控制怎样被权限、工具、门禁和恢复动作执行？ |

因此，SCOPE-V 的职责是组织运行时顺序和回流，而不是再次解释三大机制。最小映射如下：

| 自治机制 | 主要控制面 | 关键出口 |
|---|---|---|
| 意图注入 | S、C、O | 形成已签署契约和可治理上下文，才能进入执行 |
| 高频对抗与自净化 | P⇄E | 用原判据重验；超出预算、范围或授权就停止 |
| 人类异常裁决 | 贯穿 S/C/O/P/E，集中于 V | 处理价值取舍、约束例外、不可逆操作和残余风险 |

Telemetry 不是第七个控制面，而是对多次任务的慢外环；流水线不是控制面，而是把门禁、证据和发布承载到交付系统。由此形成清晰分工：**SCOPE-V 负责任务控制，三大机制负责自治原则，Harness 负责机器执行。**
