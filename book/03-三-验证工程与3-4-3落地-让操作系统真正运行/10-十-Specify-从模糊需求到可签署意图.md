...
## 10.2 批判性校准与 Grill-Me：只追问会改变结果的问题

批判性思维不是反对用户，而是拒绝把第一句话直接当成执行规格。OA 只需要检查四类会改变结果的风险：
...
Grill-Me 是把这些风险转成可选择的决定。它不是一份长问卷：每轮只问一个会显著改变价值、范围、风险、验收或权限的问题，并给出推荐默认、理由和备选项。推荐答案不是替 IO 决策，而是降低其分析负担。
...
Grill-Me 的输出仍然只是局部决定。OA 必须把决定汇总为完整契约，IO 审阅后显式签署；逐条回答、OA 总结或工具生成 `SIGNED` 都不能代替签署。
...
复杂的 Example Mapping、边界 AC 和契约漂移控制，分别在后续小节展开。
...
## 10.5 Example Mapping：把规则推进到可执行 AC
...
关键 AC 必须在 Intent Contract 签署前形成基线。实现者不能在失败后自行降低 AC、删除断言或把边界移出范围；发生这种变化时，应按规格漂移处理并重新签署。AC 可以采用人工判定、半自动验证或自动运行三种协议。重要业务规则应尽量进入自动验证，同时保留确实需要人类判断的价值与体验问题。
